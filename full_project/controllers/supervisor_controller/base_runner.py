...
class RecRunner(object):
    ...
    def __init__(self, config):
        ...
        self.adj = config["adj"]
        self.q_learning = ["rddfg_cent_rw","rmfg_cent"]
        self.adj_correlation = ["rddfg_cent_rw","rmfg_cent"]
        ...
        if self.algorithm_name == "rddfg_cent_rw":
            from r_ddfg_cent_rw.algorithm.adj_generator_new import Adj_Generator as Adj
            from r_ddfg_cent_rw.algorithm.rDDFGPolicy import R_DDFGPolicy as Policy
            from r_ddfg_cent_rw.r_ddfg import R_DDFG as TrainAlgo
        else:
    ...
    def run(self):
        ...
        # log
        if self.total_env_steps > 20000 and ((self.total_env_steps - self.last_log_T) / self.log_interval) >= 1:
